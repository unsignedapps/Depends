...
public struct TestDependency<DependencyType, Proto> where DependencyType: KeyedDependency, Proto == DependencyType.DependencyProtocol {
    ...
    public static subscript<EnclosingSelf> (
        ...
        get {
            let registered = enclosingSelf.dependencies.dependency(for: DependencyType.dependencyKey)
        ...
        set {
            if let dependency = newValue as? Proto {
                enclosingSelf.dependencies.register(dependency, for: DependencyType.dependencyKey)
                return
            } else if let dependency = newValue as? Proto?, dependency == nil {
                enclosingSelf.dependencies.unregister(key: DependencyType.dependencyKey)
                return
            }
